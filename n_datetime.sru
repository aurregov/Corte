HA$PBExportHeader$n_datetime.sru
$PBExportComments$Clase de servicio para Date y/o Datetime
forward
global type n_datetime from nonvisualobject
end type
end forward

global type n_datetime from nonvisualobject autoinstantiate
end type
...
String	is_day[7] = {"Lunes", "Martes", "Miercoles", "Jueves", "Viernes", "Sabado", "Domingo"}

// Default number of days in each month.  
// Leap years are handled in each function.
Long	ii_DaysInMonth[12] = {31,28,31,30,31,30,31,31,30,31,30,31}
...
public function long of_isoweeknumber (date ad_date)
public function long of_dayofweek (date ad_source)
public function string of_monthname (long ai_monthnumber)
public function long of_daynumber (date ad_source)
public function string of_dayname (long ai_daynumber)
public function boolean of_isweekday (date ad_source)
public function boolean of_esfestivo (date ad_dia)
end prototypes
...
Long li_adjust_months, li_adjust_years
Long li_month, li_year, li_day
Long li_temp_month
...
li_year = Long(string(ad_source,'yyyy'))
...
public function long of_isoweeknumber (date ad_date);Date ld_date
Long li_year, li_prev_year
Long li_day, li_week
...
public function long of_daynumber (date ad_source);Long li_dia
...
li_dia = DayNumber (ad_source)
li_dia --
If li_dia = 0 Then li_dia = 7

Return li_dia
end function

public function string of_dayname (long ai_daynumber);
If IsNull(ai_daynumber) or ai_daynumber < 0 or ai_daynumber > 7 Then
	Return '!' 
End If

Return is_day[ai_daynumber]
...
public function boolean of_esfestivo (date ad_dia);Date ld_dia

Select fec_fer Into :ld_dia
	From feriados
		Where fec_fer = :ad_dia ;
