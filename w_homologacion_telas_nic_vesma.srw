HA$PBExportHeader$w_homologacion_telas_nic_vesma.srw
forward
global type w_homologacion_telas_nic_vesma from w_base_simple
end type
end forward
...
integer width = 978
integer height = 2104
string title = "Homologaci$$HEX1$$f300$$ENDHEX$$n Telas Nicole Vesma"
end type
global w_homologacion_telas_nic_vesma w_homologacion_telas_nic_vesma

on w_homologacion_telas_nic_vesma.create
call super::create
end on

on w_homologacion_telas_nic_vesma.destroy
call super::destroy
...
type dw_maestro from w_base_simple`dw_maestro within w_homologacion_telas_nic_vesma
integer width = 791
integer height = 1832
string dataobject = "dgr_homologacion_tela_nic_vesma"
end type
...
choose case dwo.name
	case 'co_tela_vesma'
		...
		SELECT count(*)
		  INTO :ll_count
		  FROM h_telas
		 WHERE co_reftel = :ll_tela;
		 
		IF sqlca.sqlcode = -1 THEN
			MessageBox('Error','Ocurri$$HEX2$$f3002000$$ENDHEX$$un error al momento de verificar la tela de vesma.')
			This.SetItem(row,'co_tela_vesma',0)
			Return 2
		END IF
		
		IF ll_count = 0 THEN
			MessageBox('Error','La c$$HEX1$$f300$$ENDHEX$$digo de tela vesma no existe.')
			This.SetItem(row,'co_tela_vesma',0)
			Return 2
		END IF
end choose
