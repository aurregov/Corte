HA$PBExportHeader$w_asignacion_corte1.srw
$PBExportComments$administraci$$HEX1$$f300$$ENDHEX$$n de asignaci$$HEX1$$f300$$ENDHEX$$n de m$$HEX1$$f300$$ENDHEX$$dulos
forward
global type w_asignacion_corte1 from w_base_maestroff_detalletb
end type
type cb_1 from commandbutton within w_asignacion_corte1
end type
type cb_2 from commandbutton within w_asignacion_corte1
end type
type cb_3 from commandbutton within w_asignacion_corte1
end type
type gb_1 from groupbox within w_asignacion_corte1
end type
type gb_2 from groupbox within w_asignacion_corte1
end type
end forward
...
global w_asignacion_corte1 w_asignacion_corte1
...
Long	ii_repriorizar,ii_incremento, ii_subcentro
Long	ii_fabrica, ii_planta, ii_tipoproducto, ii_centro, ii_tipo, ii_estado
STRING   is_body,is_llamado
...
dw_detalle.Retrieve(ii_fabrica, ii_planta, 1, ii_tipoproducto, ii_centro, ii_tipo, ii_estado, gstr_info_usuario.co_planta_pro)
...
ls_nu_cut					= dw_detalle.GetItemString(il_fila_actual_detalle,"nu_cut")
...
lstr_parametros.entero[6]			=	ll_cs_liberacion
lstr_parametros.cadena[1]			=	ls_nu_orden
lstr_parametros.cadena[2]			=	ls_nu_cut
lstr_parametros.entero[7]			=	li_co_fabrica_ref
lstr_parametros.entero[8]			=	li_co_linea_ref
lstr_parametros.entero[9]			=	ll_co_referencia
lstr_parametros.entero[10]			=	ll_color_pt
lstr_parametros.cadena[3]			=	ls_tono
lstr_parametros.entero[11]			=	li_cs_particion
...
LONG					ll_ca_base_dia_prog,ll_ca_base_dia_prod,ll_inserto_tallas, ll_ordenpdn
...
STRING				ls_gpa,ls_nu_orden,ls_tono,ls_tipo_pedido,ls_de_modulo, ls_nu_cut
	...
	IF il_fila_actual_maestro > 0 THEN
		...
		ELSE
			...
			ELSE
				...
				ll_ordenpdn					= dw_detalle.GetitemNumber(il_fila_actual_detalle,"cs_ordenpd_pt")
						...
						IF ((il_fila_actual_detalle<> -1) and (NOT ISNULL (il_fila_actual_detalle)) and (il_fila_actual_detalle<>0))THEN
							...
							SELECT   dt_pdnxmodulo.ind_cambio_estilo,	dt_pdnxmodulo.orige_uni_base_dia,   
								...
								WHERE ( dt_pdnxmodulo.simulacion 			=:li_simulacion  ) AND  
										...
										( dt_pdnxmodulo.nu_cut					= :ls_nu_cut ) AND
							...
							dw_detalle.SetItem(il_fila_actual_detalle,"nu_cut",ls_nu_cut)
							...
							dw_detalle.SetItem(il_fila_actual_detalle,"cs_ordenpd_pt", ll_ordenpdn)
							...
							lstr_parametros.cadena[1] 		=  ls_nu_orden 
							lstr_parametros.cadena[3]		=	ls_nu_cut
							lstr_parametros.entero[2] 		=  li_co_fabrica_ref
							lstr_parametros.entero[3] 		=  li_co_linea_ref 
							lstr_parametros.entero[4] 		=  ll_co_referencia
							lstr_parametros.entero[5] 		=  ll_color_pt
							lstr_parametros.entero[6]		=	li_simulacion
							lstr_parametros.entero[7]		=	li_co_fabrica_modu_ant
							lstr_parametros.entero[8]		=	li_co_planta_modu_ant			
							lstr_parametros.entero[0]		=	li_co_modulo_ant			
							lstr_parametros.entero[10]		=	ll_cs_liberacion
							lstr_parametros.cadena[2]		=	ls_tono				
							lstr_parametros.entero[11]		=	li_cs_particion
							lstr_parametros.entero[12]		=	ll_ca_a_sacar
							lstr_parametros.entero[13]		=	li_co_fabrica_modu
							lstr_parametros.entero[14]		=	li_co_planta_modu			
							lstr_parametros.entero[15]		=	li_co_modulo
							lstr_parametros.entero[16]		=	1
							lstr_parametros.entero[17]		=	li_cs_particion_anterior
...
// -----------------------------------------------
							...
							IF li_co_tipo_asignacion=2 THEN 
								...
								WHERE ( dt_programa_diario.simulacion 				=:li_simulacion  ) AND  
										...
										( dt_programa_diario.nu_cut					=:ls_nu_cut ) AND
								...
								UPDATE 	dt_talla_pdnmodulo
									...
									WHERE ( dt_talla_pdnmodulo.simulacion 				=:li_simulacion ) AND  
											...
											( dt_talla_pdnmodulo.nu_cut					=:ls_nu_cut ) AND
							...
							ELSE
								CHOOSE CASE ll_hallados
									...
									CASE ELSE
										...
										ll_hallados = dw_detalle.Retrieve(li_co_fabrica_modu_ant,li_co_planta_modu_ant,li_co_modulo_ant,li_simulacion, ii_tipoproducto, ii_centro, ii_tipo, ii_estado, gstr_info_usuario.co_planta_pro)					
										IF isnull(ll_hallados) THEN
											MessageBox("Error buscando","parametros nulos",Exclamation!,Ok!)
										ELSE
											CHOOSE CASE ll_hallados
...
If isnull(ll_result) or ll_result < 1 Then 
	MessageBox('Advertencia','Ocurrio un error al momento de establecer la informaci$$HEX1$$f300$$ENDHEX$$n inicial, para el recalculo de fechas')
	Return
End if
...
luo_param.il_vector[6] = dw_detalle.GetItemNumber(ll_fila,"cs_liberacion")
luo_param.il_vector[7] = dw_detalle.GetItemNumber(ll_fila,"co_fabrica_pt")
luo_param.il_vector[8] = dw_detalle.GetItemNumber(ll_fila,"co_linea")
luo_param.il_vector[9] = dw_detalle.GetItemNumber(ll_fila,"co_referencia")
luo_param.il_vector[10] = dw_detalle.GetItemNumber(ll_fila,"co_color_pt")
luo_param.il_vector[11] = dw_detalle.GetItemNumber(ll_fila,"cs_particion")
luo_param.is_vector[1] = dw_detalle.GetItemString(ll_fila,"po")
luo_param.is_vector[2] = dw_detalle.GetItemString(ll_fila,"tono")
luo_param.is_vector[3] = dw_detalle.GetItemString(ll_fila,"nu_cut")
...
lds_mod.Retrieve(ii_fabrica, ii_planta, 1, ii_tipoproducto, ii_centro, ii_tipo, ii_estado)
...
If ll_result > 0 Then
	For ll_fila = 1 To lds_mod.RowCount()
		...
		If f_actualizar_prioridades(ii_simulacion, ii_fabrica, ii_planta, ll_mod) = -1 Then
			Rollback;
			Return
		End if
	Next
...
If ll_fila > 0 Then
	lstr_parametros.entero[1] = dw_detalle.GetItemNumber(ll_fila,'co_fabrica_exp')
	lstr_parametros.cadena[1] = dw_detalle.GetItemString(ll_fila,'po')
...
// La f$$HEX1$$e100$$ENDHEX$$brica de la tela se va a dejar como constante dentro de la orden de corte.
ll_fab = luo_constantes.of_consultar_numerico("FABRICA TELA")

IF ll_fab = -1 THEN
	Return -1
END IF
// Para Vestimundo el tono siempre va a ser 'A' mientras se mira como se implementa
ls_tono = luo_constantes.of_consultar_texto("TONO")

IF Len(ls_tono) = 0 THEN
	Return -1
END IF

ldc_porc_desperdicio = luo_constantes.of_consultar_numerico("DESPERDICIO OC")
IF ldc_porc_desperdicio = -1 THEN
	Return -1
END IF

//Data store para saber las escalas que posee una orden de corte
lds_telasxoc = Create DataStore
lds_telasxoc.DataObject = 'ds_cargar_telasxoc1'
...
FOR ll_fila = 1 TO dw_detalle.RowCount()
	ll_est_new = dw_detalle.GetItemNumber(ll_fila,'espejo_asignacion')
	...
	IF ll_est_new = 6 THEN
		...
		IF ll_ordcorte > 1  THEN
				...
				ELSE
					...
					IF Sqlca.Sqlcode = -1 THEN
						MessageBox('Error','Ocurrio un error al momento de consultar la priroridad en dt_pdnxmodulo')
						Return -1
					END IF
					...
					FOR ll_i = 1 TO lds_telasxoc.RowCount()
						...
						ll_reftel = lds_telasxoc.GetITemNumber(ll_i,'dt_telaxmodelo_lib_co_reftel')
						ll_caract = lds_telasxoc.GetITemNumber(ll_i,'dt_telaxmodelo_lib_co_caract')
						ll_diamet = lds_telasxoc.GetITemNumber(ll_i,'dt_telaxmodelo_lib_diametro')
						ll_color  = lds_telasxoc.GetITemNumber(ll_i,'dt_telaxmodelo_lib_co_color_tela')
						ls_tono   = lds_telasxoc.GetITemString(ll_i,'dt_pdnxmodulo_tono')
						ldc_cant  = lds_telasxoc.GetITemNumber(ll_i,'dt_telaxmodelo_lib_yield') * lds_telasxoc.GetITemNumber(ll_i,'dt_talla_pdnmodulo_ca_pendiente')
...
on w_asignacion_corte1.destroy
call super::destroy
				...
				CASE ELSE
					...
					ELSE
						IF  ISNULL(li_simulacion) THEN
								dw_detalle.Reset()
						ELSE
							ll_hallados = dw_detalle.Retrieve(ii_fabrica, ii_planta, 1, ii_tipoproducto, ii_centro, ii_tipo, ii_estado, gstr_info_usuario.co_planta_pro)					
							IF isnull(ll_hallados) THEN
								MessageBox("Error buscando","parametros nulos",Exclamation!,Ok!)
							ELSE
								CHOOSE CASE ll_hallados
...
event open;Long 	li_modulo
TRANSACTION			itr_tela
STRING	ls_instruccion
n_cts_constantes luo_constantes
DataWindowChild  ldc_modulos
...
//jcrm
//marzo 27 de 2007
IF gstr_info_usuario.codigo_perfil <> 16 OR gstr_info_usuario.codigo_perfil <> 5 Then
ELSE
	...
	m_mantenimiento_asignacion_modulos.m_edicion.m_fecharequisito.toolbaritemvisible = False
	...
	dw_detalle.Object.DataWindow.ReadOnly
END IF
...
IF ii_centro = -1 THEN
	Return
END IF
ii_subcentro = luo_constantes.of_consultar_numerico("SUBCENTRO INI CORTE ")
IF ii_subcentro = -1 THEN
	Return
END IF
ii_tipo = luo_constantes.of_consultar_numerico("TIPO ASIGNA")
IF ii_tipo = -1 THEN
	Return
END IF
ii_estado = luo_constantes.of_consultar_numerico("ESTADO PROGRAMACION")
IF ii_estado = -1 THEN
	Return
END IF
...
//f_rcpra_dtos_dddw_param1(dw_detalle, "co_modulo", itr_tela, ii_fabrica, ii_planta, ii_tipoproducto, ii_centro)

dw_detalle.GetChild('co_modulo', ldc_modulos)

ldc_modulos.SetTransObject(SQLCA)

ldc_modulos.Retrieve(ii_fabrica, ii_planta, ii_tipoproducto, ii_centro, ii_subcentro)
...
dw_maestro.Retrieve(ii_fabrica, ii_planta, li_modulo)
...
LONG					ll_pedido,ll_co_referencia,ll_color_pt,ll_cs_liberacion,ll_ca_unidades,ll_pedido_po
Long				li_cs_estilocolton,li_co_tipo_asignacion,li_cs_particion
STRING				ls_gpa,ls_nu_orden,ls_tono,ls_tipo_pedido,ls_sqlerr, ls_nu_cut
...
event resize;call super::resize;dw_detalle.Resize(This.Width - 120, This.Height - 400)
gb_1.Resize(This.Width - 120, This.Height - 400)
end event

type dw_maestro from w_base_maestroff_detalletb`dw_maestro within w_asignacion_corte1
integer x = 50
integer y = 52
integer width = 2729
integer height = 204
...
type dw_detalle from w_base_maestroff_detalletb`dw_detalle within w_asignacion_corte1
integer x = 5
integer y = 324
integer width = 3575
integer height = 1404
...
event dw_detalle::itemchanged;Long	li_origen_uni_base_dia,li_simulacion,li_secuenc,ll_ca_unidadesxtalla,li_ca_producida,ll_ca_numerada,ldc_cnt,&
		   ldc_pendiente,ll_co_est_prog_dia,li_fabexp,li_fabpt,li_lin,li_csest,li_cspar,li_ca_proyectada,&
			li_ca_actual,li_ca_numerada,li_proceso,li_co_estado_asigna,li_co_curva,li_co_tipo_asignacion,&
			li_ca_personasxmod,li_ind_cambio_estilo,li_tipo_empate,li_metodo_programa,li_tipo_fe_prog,li_indicativo_base,&
			li_fuente_dato,li_co_talla,li_cod_tur,li_fabrica, li_planta,li_fabricaexp,li_fabricapt,li_linea,li_estado,&
			li_tipo, li_linea_exp
LONG 		li_secuencia,ll_cs_propiedad,ll_co_est_prog_talla,ll_fuente_dato,ll_dia_curva,ll_ca_actual,ll_ca_pendiente,&
			ll_ca_base_dia_prod,ll_ca_base_dia_prog,ll_ca_a_programar,ll_ca_proyectada,ll_i,ll_pedido,ll_ref,ll_programado,&
			ll_duracion,ll_undpsb,ll_modulo_viejo,ll_cs_liberacion,ll_pedido_po,ll_planta,ll_fabrica,ll_simulacion,&
			ll_ca_programada,ll_ca_producida,ll_ca_personasxmod,ll_ca_unidad_base_dia,ll_co_modulo_fila,ll_ca_unid_base_dia,&
			ll_estado_asigna,ll_co_modulo,ll_agrupa_des,ll_personasxmodulo,ll_unidades_empate,ll_co_metodo_programa,&
			ll_co_consumo_proceso,ll_unidad_consum_est,ll_unidade_consum_std,ll_agrupa,li_prdact,li_prdnue,ll_filas,&
			ll_count,ll_estado_asigna2,ll_estado_asigna3,ll_modulo,ll_referencia,ll_colorpt,ll_cs_estilo,ll_cs_particion,&
		   ll_area,ll_cs_observacion,ll_asignacion,ll_agrupacion,li_cslib,li_cs_prioridad, ll_ordenpdn, ll_unir_oc,&
			ll_pedido_pdp, ll_col
STRING   ls_sqlerr,ls_expresion,ls_usuario,ls_instancia,ls_co_estado_merc,ls_po,ls_tono,ls_body,ls_observacion
String	ls_cut, ls_ref_exp, ls_color_exp
DATETIME	ldt_fe_inicio_prog,ldt_fe_fin_prog, ldt_fe_requerida_desp,ldt_fe_entra_pdn,ldt_fe_lim_prog,ldt_fecha_final,&
			ldt_fecha_inicial,ldt_inicio,ldt_fin,ldt_fe_desp_programada,ldt_fe_creacion,ldt_fe_actualizacion
DECIMAL	ld_porc_eficiencia,ldc_mindspini,ldc_mindspfn,ldc_min,ldc_ca_minutos_std,ldc_minutos_jornada
s_base_parametros lstr_parametro
SetPointer(Hourglass!)
...
IF dwo.Name = 'co_modulo' Then
	...
	//			ll_i = row
				...
				ls_cut				= dw_detalle.GetItemString(ll_i, "nu_cut")
				...
				ll_col    			= dw_detalle.GetitemNumber(ll_i,"co_color_pt")   
				ls_tono   			= dw_detalle.GetitemString(ll_i,"tono")   
				li_cspar	 			= dw_detalle.GetitemNumber(ll_i,"cs_particion")
				
				
				ls_expresion = "co_fabrica_exp = " + String(li_fabexp) + " and cs_liberacion = " &
									+ String(li_cslib) +  " and po = '" + ls_po + "'" + " and nu_cut = '" + ls_cut + "' and co_fabrica_pt = " + String(li_fabpt) +&
									" and co_linea = " + String(li_lin) + " and co_referencia = " + String(ll_ref) + &
									" and co_color_pt = " + String(ll_col) + " and tono = '" + ls_tono + "'" + &
									" and simulacion = " + String(ll_simulacion) + &
				...
				SELECT cs_prioridad,ca_programada,ca_producida,ca_pendiente,co_estado_asigna,   
						...
						ca_base_dia_prog,ca_a_programar,cs_asignacion, cs_ordenpd_pt, co_linea_exp,
						co_ref_exp, co_color_exp, cs_unir_oc, pedido
				INTO :li_cs_prioridad,:ll_ca_programada,:ll_ca_producida,:ll_ca_pendiente,   
						...
						:li_indicativo_base,:ll_ca_base_dia_prod,:ll_ca_base_dia_prog,:ll_ca_a_programar,
						:ll_asignacion, :ll_ordenpdn, :li_linea_exp, :ls_ref_exp, :ls_color_exp, :ll_unir_oc, 
						:ll_pedido_pdp
				FROM dt_pdnxmodulo  
			 ...
			 INSERT INTO dt_pdnxmodulo 
						( simulacion,co_fabrica,co_planta,co_modulo,co_fabrica_exp,cs_liberacion,   
						  po,nu_cut,co_fabrica_pt,co_linea,co_referencia,co_color_pt,tono,   
						  cs_particion,cs_prioridad,ca_programada,ca_producida,ca_pendiente,   
						  ...
						  ca_base_dia_prod,ca_base_dia_prog,ca_a_programar,cs_asignacion, cs_ordenpd_pt, co_fabrica_prop,
						  co_linea_exp, co_ref_exp, co_color_exp, cs_unir_oc, pedido	)  
			  VALUES ( :ll_simulacion,:ll_fabrica,:ll_planta,:ll_co_modulo,:li_fabexp,   
						  :li_cslib,:ls_po,:ls_cut,:li_fabpt,:li_lin,:ll_ref,   
						  :ll_col,:ls_tono,:li_cspar,   
						  ...
						  :ll_ca_a_programar,:ll_asignacion, :ll_ordenpdn, :gstr_info_usuario.co_planta_pro,
						  :li_linea_exp, :ls_ref_exp, :ls_color_exp, :ll_unir_oc, :ll_pedido_pdp)  ;
				IF SQLCA.SQLCODE=-1 THEN
...
//					END IF
				...
				DO WHILE SQLCA.SQLCode = 0
					INSERT INTO dt_talla_pdnmodulo  
							( simulacion,co_fabrica,co_planta,co_modulo,co_fabrica_exp,cs_liberacion,   
							  po,nu_cut,co_fabrica_pt,co_linea,co_referencia,co_color_pt,tono,cs_particion,   
					 ...
					 VALUES ( :ll_simulacion,:ll_fabrica,:ll_planta,:ll_co_modulo,   
							  :li_fabexp,:li_cslib,:ls_po,:ls_cut,:li_fabpt,:li_lin,:ll_ref,:ll_col,:ls_tono,
				...
				DO WHILE SQLCA.SQLCode = 0
					INSERT INTO dt_programa_diario  
						  ( simulacion,co_fabrica,co_planta,co_modulo,co_fabrica_exp,
							 ...
							 tono,cs_particion,cs_fechas,fe_inicio,ca_unid_dispon_ini,   
							 ca_min_dispon_ini,ca_unid_dispon_fin,ca_min_dispon_fin,personasxmodulo,   
							 dia_curva,porc_eficiencia,ca_unid_posibles,duracion,fe_fin,ca_programada,   
							 ca_producida,ca_pendiente,co_est_prog_dia,fuente_dato,fe_creacion,   
							 fe_actualizacion,usuario,instancia,ca_proyectada,ca_actual,ca_numerada )  
					 VALUES ( :ll_simulacion,:ll_fabrica,:ll_planta,:ll_co_modulo,:li_fabexp,
							 :li_cslib,:ls_po,:ls_cut,:li_fabpt,:li_lin,:ll_ref,:ll_col,:ls_tono,
				...
				DO WHILE SQLCA.SQLCode = 0
					INSERT INTO dt_procesos_plan  
						( simulacion,co_fabrica,co_planta,co_modulo,co_fabrica_exp,   
						  cs_liberacion,po,nu_cut,co_fabrica_pt,co_linea,   
						  co_referencia,co_color_pt,tono,cs_particion,   
						  co_proces,cs_proceso,co_metodo_programa,co_consumo_proceso,ca_consumo_estimad,   
						  unidad_consum_est,estandar_consumo,unidade_consum_std,fe_creacion,   
						  fe_actualizacion,usuario,instancia )  
					VALUES ( :ll_simulacion,:ll_fabrica,:ll_planta,:ll_co_modulo,:li_fabexp,   
						  :li_cslib,:ls_po,:ls_cut,:li_fabpt,:li_lin,:ll_ref,:ll_col,   
						  :ls_tono,:li_cspar,:li_proceso,:li_secuenc,:ll_co_metodo_programa,:ll_co_consumo_proceso,:ldc_cnt,   
						  :ll_unidad_consum_est,:ldc_min,:ll_unidade_consum_std,:ldt_fe_creacion,:ldt_fe_actualizacion,:ls_usuario,:ls_instancia )  ;
...
//				SELECT simulacion,co_fabrica,co_planta,co_fabrica_exp,cs_liberacion,po,nu_cut,
//						co_fabrica_pt,co_linea,co_referencia,co_color_pt,tono,cs_particion,   
...
//				END IF
//				FETCH cur_obser
//				INTO :li_simulacion,:li_fabrica,:li_planta,:li_fabricaexp,:ll_cs_liberacion,:ls_po,:ls_cut,:li_fabricapt,
//					  :li_linea,:ll_referencia,:ll_colorpt,:ls_tono,:ll_cs_particion,:ls_observacion,:ll_area
//					  ,:ll_cs_observacion,:ldt_fe_actualizacion,:ldt_fe_creacion,:ls_usuario,:ls_instancia;
//						 
//				DO WHILE SQLCA.SQLCode = 0
//					
//					INSERT INTO dt_observaxmodulo  
//						( simulacion,co_fabrica,co_planta,co_modulo,co_fabrica_exp,cs_liberacion,po, nu_cut,
//						  co_fabrica_pt,co_linea,co_referencia,co_color_pt,tono,cs_particion,   
//						  observacion,area,cs_observacion,fe_actualizacion,fe_creacion,usuario,instancia )  
//					VALUES ( :li_simulacion,:li_fabrica,:li_planta,:ll_co_modulo,:li_fabricaexp,:ll_cs_liberacion,
//							:ls_po,:ls_cut,:li_fabricapt,:li_linea,:ll_referencia,:ll_colorpt,:ls_tono,:ll_cs_particion,
...
//			
//					FETCH cur_obser
//					INTO :li_simulacion,:li_fabrica,:li_planta,:li_fabricaexp,:ll_cs_liberacion,:ls_po,:li_fabricapt,
//						  :li_linea,:ll_referencia,:ll_colorpt,:ls_tono,:ll_cs_particion,:ls_observacion,:ll_area
//						  ,:ll_cs_observacion,:ldt_fe_actualizacion,:ldt_fe_creacion,:ls_usuario,:ls_instancia;
//				LOOP
//			
...
//				END IF	
		...
		this.Accepttext()
	ELSE
		MessageBox("Advertencia...","No puede cambiar una producci$$HEX1$$f300$$ENDHEX$$n que no ha sido aprobada")
		Return 2
	END IF
...
IF dwo.Name = 'co_estado_asigna' THEN
	...
	Long ll_orden,ll_estado_new,ll_estado_old
	ll_estado_new = long(data)
	ll_estado_old = This.GetItemNumber(row,'co_estado_asigna')
	
	//----------------------------se verifica que no se valla a colocar un estado mayor a 6
	If ll_estado_old <= 6 AND ll_estado_new > 6 Then
	 	 MessageBox('Error','Esta intentando mover el registro a un estado que no es posible en esta etapa.')
		 This.SetItem(row,'co_estado_asigna',ll_estado_old) 
		 This.AcceptText()
		 Return 2
	End if
	...
	IF ll_agrupa <> 0 THEN
	//para que al momento de aprobar una orden con estado superior conserve el estado mayor, pero el sistema
	//debe reconocer que la oc, fue aprobada, enviar correo y cargar en dt_telasxoc 
		If ll_estado_old > 6 Then
			This.SetItem(row,'co_estado_asigna',ll_estado_old)
			ll_modulo = This.GetItemNumber(row,'co_modulo')
			ll_estado_asigna = Long(Data)
			ll_estado_asigna3 = This.GetItemNumber(row,'espejo_asignacion')
			IF ll_modulo <> 0  THEN
				FOR ll_i = 1 TO This.RowCount()
					This.AcceptText()
					ll_estado_asigna2 = This.GetItemNumber(ll_i,'espejo_asignacion')
					IF ll_agrupa = This.GetItemNumber(ll_i,'dt_agrupa_pdn_cs_agrupacion') THEN
						This.SetItem(ll_i,'espejo_asignacion',ll_estado_asigna)
					END IF//ll_agrupa
					IF ll_estado_asigna2 = 6 THEN
						...
						IF IsNull(ldt_fecha_inicial) or Isnull(ldt_fecha_final) THEN
							...
							This.SetItem(ll_i,'espejo_asignacion',ll_estado_asigna3)
							This.AcceptText()
							Return 2
						END IF
			...
			This.SetItem(row,'co_estado_asigna',ll_estado_old)
			Return 2
		End if//ll_estado_old > 6
	
		If ll_estado_old >= 10 And ll_estado_new < 10 Then
			MessageBox('Error','La O.C. ya no puede ser devuelta, debido a que el proceso se encuentra muy avanzado')
			Rollback;
			Return
		End if
		...
		If ll_estado_old >= 6 And ll_estado_new < 6 Then
			...
			ll_col    			= dw_detalle.GetitemNumber(row,"co_color_pt")   
...
//				End if
			...
			//envio correo informando la cancelaci$$HEX1$$f300$$ENDHEX$$n
			ls_body = "Orden Corte: "+ String(ll_orden) + ", Pedido: "+ String(ll_pedido)+", P.O.: "+ ls_po+&
						", Linea: "+ String(li_lin) + ", Referencia: "+String(ll_ref)+", Color: "+String(ll_col)+&
						", Tono: "+ls_tono
			is_body = ls_body
			is_llamado = 'SI'
			dw_detalle.TriggerEvent(updateend!)
		End if
		...
		END IF
	Else
		MessageBox("Advertencia...","La producci$$HEX1$$f300$$ENDHEX$$n debe estar agrupada para poder cambiar el estado")
		This.SetItem(row,'co_estado_asigna',ll_estado_old)		
		Return 2
	End if		
...
String ls_po,ls_referencia,ls_color,ls_body,ls_tono
DateTime ldt_fe_ini_prog,ldt_fe_fin_prog
Long  li_modulopordefecto

DWItemStatus l_status_fila, l_status_modulo

n_cts_constantes luo_constantes

luo_constantes = Create n_cts_constantes

li_modulopordefecto = luo_constantes.of_consultar_numerico("MODULO")
If li_modulopordefecto = -1 Then
	MessageBox("Advertencia", "Fall$$HEX2$$f3002000$$ENDHEX$$la lectura de la constante -MODULO-", StopSign!)
	Return
End IF

dw_detalle.AcceptText()
FOR ll_i = 1 TO dw_detalle.RowCount()
	l_status_fila = dw_detalle.GetItemStatus(ll_i, 0, Primary!)
	If l_status_fila = NewModified! OR l_status_fila = DataModified! Then
		l_status_modulo = dw_detalle.GetItemStatus(ll_i, "co_modulo", Primary!)
		If l_status_modulo = NotModified! And dw_detalle.GetItemNumber(ll_i, "co_modulo") = li_modulopordefecto Then
			If MessageBox("Validaci$$HEX1$$f300$$ENDHEX$$n", "La Orden de Corte est$$HEX2$$e1002000$$ENDHEX$$asignada al m$$HEX1$$f300$$ENDHEX$$dulo " + String(li_modulopordefecto) + ", desea continuar?", Question!, YesNo!, 2) = 2 Then
				dw_detalle.SelectRow(0, False)
				dw_detalle.SelectRow(ll_i, True)
				dw_detalle.ScrollToRow(ll_i)
				Return
			End If
		End If
	End If
...
// Se pone en comentarios porque esta funcionalidad no se est$$HEX2$$e1002000$$ENDHEX$$utilizando.
// Responsable: Jos$$HEX2$$e9002000$$ENDHEX$$Fernando Arango J
// Fecha: Mayo 16 de 2005
...
//			ls_body = " "
//		END IF
//		ls_body = ls_body + "Prioridad: " +String(ll_cs_prioridad) + &
...
////			pr_generar_mercada(:ll_cs_asignacion, 0);
////			
////		EXECUTE mercada;
////		IF SQLCA.SQLCode = -1 THEN
////			MessageBox("Error Base Datos","Error al generar la mercada de telas " + SQLCA.SQLErrText)
////			Return 1
////		END IF
//	END IF
//	ll_fila = ll_fila	+ 1
...
event dw_detalle::updateend;call super::updateend;// Se pone en comentarios porque esta funcionalidad no se est$$HEX2$$e1002000$$ENDHEX$$utilizando.
// Responsable: Jos$$HEX2$$e9002000$$ENDHEX$$Fernando Arango J
// Fecha: Mayo 16 de 2005
...
//luo_constantes = Create n_cts_constantes
//
//ls_correo = luo_constantes.of_consultar_texto("CORREO TELA")
//
...
//	
//	lole_item.To      = ls_correo
//	//lole_item.To      = "Hector Fabio Arango"
//	
//	If is_llamado = 'SI' Then
//		lole_item.Subject = "Ordenes de Corte Suspendidas"
//	Else
