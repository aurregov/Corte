HA$PBExportHeader$w_maestro_partesxprenda.srw
forward
global type w_maestro_partesxprenda from w_base_tabular
end type
end forward

global type w_maestro_partesxprenda from w_base_tabular
string title = "Partes de Prenda"
string menuname = "m_mantenimiento_simple"
end type
global w_maestro_partesxprenda w_maestro_partesxprenda

on w_maestro_partesxprenda.create
call super::create
if this.MenuName = "m_mantenimiento_simple" then this.MenuID = create m_mantenimiento_simple
end on

on w_maestro_partesxprenda.destroy
call super::destroy
if IsValid(MenuID) then destroy(MenuID)
end on

event open;call super::open;This.width = 1339
This.height = 972
end event

event ue_buscar;call super::ue_buscar;s_base_parametros lstr_parametros
long ll_hallados
string ls_de_parte_prenda
...
//	IF lstr_parametros.hay_parametros THEN
	ls_de_parte_prenda=sle_argumento.text
//		arg1=lstr_parametros.cadena[1]
//		arg2=lstr_parametros.cadena[2]
//		arg3=lstr_parametros.entero[1]

		ll_hallados = dw_maestro.Retrieve(ls_de_parte_prenda)
...
type dw_maestro from w_base_tabular`dw_maestro within w_maestro_partesxprenda
string dataobject = "dtb_m_partes_prenda"
end type

type sle_argumento from w_base_tabular`sle_argumento within w_maestro_partesxprenda
end type

type st_1 from w_base_tabular`st_1 within w_maestro_partesxprenda
end type
