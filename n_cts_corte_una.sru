HA$PBExportHeader$n_cts_corte_una.sru
forward
global type n_cts_corte_una from nonvisualobject
end type
end forward

global type n_cts_corte_una from nonvisualobject
end type
global n_cts_corte_una n_cts_corte_una
...
Do
	...
	If ll_flfnd <= 0 Then
		ls_mensaje="No se pudo encontrar los minutos disponibles para la fecha:"+String(an_inicio)
...
lds_produccion.DataObject = 'd_lista_produccion_para_programar_una'
...
//Recupero la produccion a la cual se le programara el tiempo	
       ...
       where simulacion = :ai_simulacion and 
		...
		For ll_j = 1 To lds_minxprenda.RowCount()
         ...
         values ( :ai_simulacion,:ai_fabrica,:ai_planta,:ai_modulo,:li_fabexp,   
              ...
              :ls_tono,:li_cspar,:li_proceso,:li_secuenc,1,1,:ldc_cnt,   
		...
		If ldc_totmin > 0 THEN
			Do While ldc_pendiente > 0				
				If ldc_mindspini > 0 And Truncate(ldc_mindspini / ldc_totmin, 0) > 0 Then
					...
					End If
				Else
//  Se coloca en comentario por problemas en la agrupacion y no se sabe esta parte
//  para que es, en Marinilla no se utiliza, no hay documentacion. jorodrig  julio 6 2005
//					If This.Of_MinutosCalendario(ld_inicio,ldc_mindspini) = -1 Then
//						rollback ;
//						Return -1
//					End If
					//se adicionan estas dos lineas para salir del ciclo jorodrig julio 6
						ll_programado = ldc_pendiente
						ldc_pendiente = 0
				End If
			Loop
...
on n_cts_corte_una.create
call super::create
TriggerEvent( this, "constructor" )
end on

on n_cts_corte_una.destroy
