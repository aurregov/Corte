HA$PBExportHeader$w_base_estadisticas.srw
forward
global type w_base_estadisticas from window
end type
type dw_reporte from datawindow within w_base_estadisticas
end type
end forward
...
global w_base_estadisticas w_base_estadisticas
			...
			CHOOSE CASE ls_tipo_campo
				...
				CASE	'dd/mm/yyyy hh:mm','dd/mm/yyyy'		// filtro para fechas
					   ...
					   ELSE
						...
						END IF						
				CASE 	'','#,##0.00','0','#,##0','###,###','###','#######'      //N$$HEX1$$fa00$$ENDHEX$$mericos
						IF isnull(is_filtrar) or is_filtrar = '' then
							is_filtrar = ls_columna + ls_condicion + ls_filtro			
						ELSE
							is_filtrar = is_filtrar +' AND ' + ls_columna + ls_condicion + ls_filtro			
