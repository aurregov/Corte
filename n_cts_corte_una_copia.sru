HA$PBExportHeader$n_cts_corte_una_copia.sru
forward
global type n_cts_corte_una_copia from nonvisualobject
end type
end forward

global type n_cts_corte_una_copia from nonvisualobject
end type
global n_cts_corte_una_copia n_cts_corte_una_copia

type variables
uo_dsbase ids_calendario, ids_procesos, ids_prog_diario
...
public function long of_metodominutos (long ai_simulacion, long ai_fabrica, long ai_planta, long ai_modulo, long ai_prioridad, date ad_inicio, ref uo_dtwndow adw_produccion);/*	---------------------------------------------------------------------
	...
	---------------------------------------------------------------------*/
uo_dsbase lds_produccion,lds_minxprenda,lds_fechainici
Long     ll_i,ll_j,ll_pedido,ll_ref,ll_result,ll_flfnd, ll_registro, &
         ll_programado,ll_duracion,ll_undpsb, ll_color
...
String   ls_po,ls_tono,ls_sqlerr,ls_linea,ls_estilo
...
//	Obtiene el tipo producto, centro y subcentro segun fabrica, planta y modulo
select co_tipoprod,   
       co_centro_pdn,   
       co_subcentro_pdn  
  into :li_tpprod,   
...
//              :ll_pedido,:li_cslib,:ls_po,:li_fabpt,:li_lin,:ll_ref,:ll_color,   
...
//	             :li_cslib,:ls_po,:li_fabpt,:li_lin,:ll_ref,:ll_color,:ls_tono,:li_csest,   
...
//				End If
		...
		ll_registro = adw_produccion.Find("simulacion = " + String(ai_simulacion) &
			 ...
			 +" And co_color_pt = "+String(ll_color)+" And tono = '" +ls_tono+"'" &
...
on n_cts_corte_una_copia.create
call super::create
TriggerEvent( this, "constructor" )
end on

on n_cts_corte_una_copia.destroy
