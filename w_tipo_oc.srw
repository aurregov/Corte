HA$PBExportHeader$w_tipo_oc.srw
forward
global type w_tipo_oc from window
end type
type cb_grabar from commandbutton within w_tipo_oc
end type
type dw_maestro from uo_dtwndow within w_tipo_oc
end type
end forward
...
Comentario: Se crea una nueva tabla m_cptos_corte para evitar quemar los datos del ddlb existente y se adiciona dddw_m_cptos_corte
se modifico dtb_tipo_oc para adicionarle el dddw_m_cptos_corte
***********************************************************/
This.x = 1
This.y = 1
//This.width = 
...
IF lstr_param.hay_parametros = TRUE THEN	//if hay parametros
	//carga informaci$$HEX1$$f300$$ENDHEX$$n a unir
	ll_oc = lstr_param.entero[1]
	
	IF dw_maestro.retrieve(ll_oc) <= 0 THEN
		messagebox("Aviso","No se encontr$$HEX2$$f3002000$$ENDHEX$$informaci$$HEX1$$f300$$ENDHEX$$n de la O.C")
	END IF
...
integer height = 328
boolean titlebar = true
string title = "Tipo O.C"
string dataobject = "dtb_tipo_oc"
